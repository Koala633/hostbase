...
with open("ssid.txt", 'r') as file:  # A METTRE le input pour choper le ssid.
    ...
    for i, number in enumerate(data):
        ...
        if not found:
            print(f"{RED}ERROR los datos de la red no han sido encontrado !")
            print(f"{YELLOW}Quisas usted no ha entrado en buen nombre o la red esta demasiado lejos")
            print(f"{RESET}Quitando...")
            os.system("rm -rf *.txt")
            os.system("rm -rf *.csv")
...
# os.system("rm -rf *f.txt") A VOIR.
    with open('finalchoice.csv') as cards:
        ...
        for index, row in enumerate(csv_reader):
            if index == 0:  # déplacement vertical dans ce cas la ligne 1 Livebox-D380
                print(f"{CYAN}Bssid de la red en 2.4GHz :")
    ...
    with open('finalchoice.csv') as cards:
        ...
        for index, row in enumerate(csv_reader):
            if index == 0:  # déplacement vertical dans ce cas la ligne 1 Livebox-D380
                print(f"{CYAN}Canal de la red en 2.4GHz :")
    ...
    with open('finalchoice.csv') as cards:
        ...
        for index, row in enumerate(csv_reader):
            if index == 0:  # déplacement vertical dans ce cas la ligne 1 Livebox-D380
                print(f"{CYAN}Essid de la red en 2.4GHz :")
...
if os.path.isfile('airodumpwpacinqg-01.csv'):
    ...
    with open('midvalues1.csv') as cards:  # Placement des données dans les fichiers textes sans caractères inutiles après le second filtre sur midvalues1.csv
        ...
        for index, row in enumerate(csv_reader):
            ssid.write(f"{row[4]}\n")
    print(f"{RESET}Guardando los datos de la red en 5GHz...")
    cinqfiltre = subprocess.Popen("python filtre1.py", shell=True)
    sys.exit()
else:
