...
Process.kill 15, File.read('/tmp/hostbase-1.5ES/wash.pid').to_i
`killall wash`
if File.exist?("terminalfrequence.pid")
sleep 1
Process.kill 15, File.read('/tmp/hostbase-1.5ES/terminalfrequence.pid').to_i
`killall wash`
else
nil
end
...
puts "\e[1;32m[*] La clave wifi WPA ha sido capturada y registrado en la carpeta: /etc/wpa_supplicant.conf\e[0m"
puts "\e[1;32m[*] Comando para ver la clave wifi WPA: cat /etc/wpa_supplicant.conf\e[0m"
