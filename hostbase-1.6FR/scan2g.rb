...
system"airmon-ng"
puts "Entrez une carte wifi pour le scan de réseau en 2.4GHz: "
$wlan_name = gets.chomp
puts "#{$wlan_name} a été sélectionnée pour le scan"
...
def select_point_to_crack(access_points, wlan_iface_name)
  ...
  print "Entrez un nombre (Les nombres commençants par 0, 1 etc... siginifient que les réseaux sont proches):"
 ...
 `echo #{point_to_crack[:essid]} > ssid.txt`
  cmd = "ruby init.rb"
  puts "Le fake AP aura comme nom #{point_to_crack[:essid]}"
  exec(cmd)
end
