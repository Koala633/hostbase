...
system("ruby wpsexit.rb")
 ...
 load 'metcatcheck.rb'
  ...
  Process.kill 15, File.read('/tmp/hostbase-1.6FR/wash.pid').to_i
  `killall wash`
  if File.exist?("terminalfrequence.pid")
  sleep 1
  Process.kill 15, File.read('/tmp/hostbase-1.6FR/terminalfrequence.pid').to_i
  `killall wash`
  else
  nil
  end
  ...
  `cp -R /tmp/hostbase-1.6FR/wpa_supplicant.conf /etc/`
  sleep 1;
